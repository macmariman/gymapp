import { formatDurationInputValue } from "@/lib/workouts/duration"
import type {
  AttendanceMonth,
...
function formatDurationUnit(
  ...
  if (durationFormat === "mmss") {
    return formatDurationInputValue(seconds, durationFormat)
  }
...
export function formatTarget(
  exercise: Pick<
    ExerciseView,
    "targetType" | "targetValue" | "note" | "durationFormat"
  >
) {
  const targetLabel =
    exercise.targetType === "time"
      ? formatDurationUnit(exercise.targetValue, exercise.durationFormat)
      : exercise.targetType === "weight"
        ? `${exercise.targetValue} kg`
        : `${exercise.targetValue} rep`

  return `${targetLabel}${exercise.note ? ` ${exercise.note}` : ""}`
}
...
export function formatDuration(seconds: string | number) {
  return formatDurationUnit(seconds)
}

export function formatDurationWithFormat(
  seconds: string | number,
  durationFormat: ExerciseDurationFormat = "seconds"
) {
  return formatDurationUnit(seconds, durationFormat)
}
...
export function formatLoggedValue(
  ...
  if (logType === "time") {
    return formatDurationWithFormat(value, durationFormat)
  }
...
export function formatLogSummary(
  ...
  const formattedValues = values.map((value) => {
    if (logType === "time") {
      return formatDurationInputValue(value, durationFormat)
    }
...
export function buildAttendanceGrid(attendance: AttendanceMonth) {
  ...
  const totalDays = new Date(
    Date.UTC(attendance.year, attendance.month, 0)
  ).getUTCDate()
